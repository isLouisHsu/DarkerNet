...
layer make_connected_layer(int batch, int inputs, int outputs, ACTIVATION activation, int batch_normalize, int adam)
{
    ...
    fprintf(stderr, "connected   %4d x%4d  %4d x%4d x%4d   ->  %4d x%4d x%4d\n", l.inputs, l.outputs, l.h, l.w, l.c, l.out_h, l.out_w, l.out_c);
...
void forward_connected_layer(layer l, network net)
{
    ...
    int k = l.inputs;
    int n = l.outputs;
    float *a = net.input;   // m x k
    float *b = l.weights;   // n x k
    float *c = l.output;    // m x n
    gemm(0,1,m,n,k,1,a,k,b,k,1,c,n);    // c = a b^T
    if(l.batch_normalize){
